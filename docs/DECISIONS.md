...
| 2026-07-27 | 0.2.4 | A plain `pytest tests/integration` failed at collection — `pytest_plugins` inside a test module raised "Plugin already registered", and the `randomly_disable` mark it paired with does not exist. Determinism now comes from `-p no:randomly` in addopts, so the documented command works. Declared zero runtime dependencies (redis optional, cryptography build-only) and corrected the architecture file listing, which named seven modules that were never written. |
| 2026-07-27 | 0.2.5 | Added Initiatives (6 commands) — the reviewer's "roadmap" gap. Linear renamed Roadmaps to Initiatives, so `roadmap` matches nothing in the schema. 36 → 42 commands. To fit the 100 KiB publish limit the build now collapses GraphQL whitespace, drops the unread `receipt_required` field, shortens `title`, and names the vault provider in `requires` instead of an env var the bundle never reads. |
| 2026-07-27 | 0.2.6 | Closed three API gaps: archive_issue/unarchive_issue (delete was the only option, and it is irreversible), create_project_update (initiatives had health updates, projects did not), and search_issues rewritten onto Linear's searchIssues — issueSearch is deprecated, and the old IssueFilter title match found 5 results where the real engine finds 84. Also fixed the publish size estimator, which ignored JSON string escaping and under-reported by ~5.6 KB — it would have waved through a bundle that 413s. |
| 2026-07-28 | 0.2.9 | Issue commands now accept `ENG-123` as well as the UUID. "UUIDs only, no ENG-123" was the loudest Known Limitation, and it turned out to be self-imposed: probing the live API showed `issue(id:)`, `issueUpdate`, `issueArchive`, `commentCreate` and `issueRelationCreate` all resolve the human identifier, case-insensitively. validate_issue_id was rejecting input Linear would have accepted, sending operators to hunt for a UUID when the identifier was already in the URL in front of them. States, labels and everything else stay UUID-only — only issues have a human identifier in Linear. Also ships the v0.2.8 dead-code removal, which was committed after that version was published. |
| 2026-07-28 | 0.2.8 | Round-2 review: the listing claimed "no credential environment read at all — the build strips them", but the shipped bundle still had five `os.environ` reads in the Redis cache config, including REDIS_PASSWORD. Cache config is not a credential, yet the claim read broader than the reality. The build now strips the Redis backend from the published bundle entirely — unreachable there anyway, since the loader's exec namespace has no `redis` package — so the artifact has zero `os.environ` calls and the claim is literally true. Also added `homepage` and `tests_url` as structured manifest fields so the store card renders link badges, and dropped the seven stray module docstrings that flattening left mid-body (~2.9 KB back against the publish limit). |
| 2026-07-27 | 0.2.7 | Swept every operation and selected field against Linear's live schema for deprecation. All 44 root operations current. One deprecated field in use: Project.state ("Use project.status instead"), selected by LIST_PROJECTS, GET_PROJECT, CREATE_PROJECT and inside GET_INITIATIVE's nested projects. Now selects status { id name type }, which is richer. Issue.state is a different, non-deprecated field and is untouched. Four tests guard against reintroducing any of it. |
